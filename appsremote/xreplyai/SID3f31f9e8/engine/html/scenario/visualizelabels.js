class VisualizeLabels
{
		...
		AllElement.forEach(function(ElementRaw){
			...
			if(Element.parents('.folded').length == 0)
			{
				...
				if(Element[0].hasAttribute("data-label-name"))
					...
					ScenarioDataResult.push(AddData)
			}
		})
		return ScenarioDataResult
	}
	...
	async Visualize()
	{
		...
		//Save id of current task, so we can understand, if new task will be started
		let CurrentVisualizeId = (Math.random() + 1).toString(36).substring(2)
		this.CurrentVisualizeId = CurrentVisualizeId
		...
		if(this.CurrentVisualizeId != CurrentVisualizeId)
		{
			//Other task has been started, so this must be finished
			return;
		}

		//Do all required reading from DOM		
		let NewScenarioData = self.CalculateScenarioData()
	...
	{
		Offset = Offset * (this.LineWidth + this.SpaceBetweenLines) + 10
