(function (window) {
  ...
  const ActionUpdaterModal = Backbone.View.extend({
        ...
        <div class="action-updater-modal-content">
          ...
          <div class="action-updater-modal-body">
            <div class="scenario-page-notify">
              <div class="scenario-page-notify-text"><%= tr('The same actions may differ in different BAS versions. To update an action to the current BAS version, you need to use this tool or recreate each action manually.') %></div>
              <div class="scenario-page-notify-text"><%= tr('Actions that are not updated may not work correctly or even cause an error.') %></div>
              <div class="scenario-page-notify-text"><%= tr('This tool allows you to automatically update all actions in the project.') %></div>
              <div class="scenario-page-notify-text"><%= tr('You can always open it in the ') %><a class="scenario-page-link" data-target="Updater"><%= tr('context menu') %></a>.</div>
            </div>
            <% if (notify) { %>
            <div class="scenario-page-notify warning">
              <div class="scenario-page-notify-text"><%= tr('The scenario panel interface has changed - to return to the <b>classic</b> theme, open the settings window through the <a class="scenario-page-link" data-target="Settings">context menu</a> and select the desired options.') %></div>
    ...
    events: {
      ...
      'click .scenario-page-link': function (e) {
        BrowserAutomationStudio_HighlightMenuItem(`Show${e.target.dataset.target}`);
      }
    },
    ...
    initialize() {
      _MainView.once('render', () => {
        ...
        if (compareVersion(_ApplicationEngineVersion, _ScriptEngineVersion) !== 0) {
          this.notify =
            compareVersion(_ApplicationEngineVersion, '26.9.0') >= 0 &&
            compareVersion(_ScriptEngineVersion, '26.8.1') <= 0;
          this.show();
        }
      });
    },
    ...
    render() {
      ...
      this.$el.html(this.template({ notify: this.notify })).modal({
        backdrop: 'static',
        keyboard: false,
        show: false,
      });
  ...
  const ActionUpdaterView = Backbone.View.extend({
    template: _.template(/*html*/`
      ...
      <div class="scenario-page-panel">
        ...
        <select id="actionUpdaterSelect">
          <option class="scenario-page-select-option" value="all" selected="selected"><%= tr('All actions in the project') %></option>
          <option class="scenario-page-select-option" value="current"><%= tr('All actions in the current function') %></option>
          <option class="scenario-page-select-option" value="selected"><%= tr('Only the selected actions') %></option>
        </select>
      ...
      <div class="scenario-page-footer updater-page-footer">
    ...
    update() {
      ...
      this.$('.scenario-page-select').trigger('blur');
