...
_G = {
   "ai": {icon:  "../icons/ai.png", info: "Generate code from user prompt."},
...
_AL = 
{
   ...
   "Generate code from user prompt.": {"ru": "Сгенерировать код из запроса пользователя."},
...
function prepare_margins(parentid, length, default_value, is_modern = false)
{
   var res = ""
   ...
   if(len > 0)
      res += "background-image: linear-gradient(to right"

   for(var i = 0;i<len;i++)
   {
      if(is_modern || lines[i])
      {
         if(is_modern)
            color = lines[i] ? '#d0d0d0' : '#f0f0f0';
         if(i == 0)
         {
         ...
         {
            res += `, rgba(0,0,0,0) ${20 * i}px, ${color} ${20 * i}px, ${color} ${20 * i + width}px, rgba(0,0,0,0) ${20 * i + width}px`
            has_gradient = true
         }
      }
   }
...
function prepare_group(dat, task_name, task_id)
{
 ...
 function prepare_title(dat, task_name, is_modern = false)
 {
   try
   {
     ...
     return (is_modern ? "" : pref) + tr(_A[dat["s"]]["name"])
   }catch(e)
   {
   }
   return ""
 ...
 function prepare_label_data_json(dat, task_name)
 {
   try
   {
     if(task_name == "Initialize")
     ...
     if(dat["s"] == "goto")
     {
      Res["data-is-label-from"] = "true"
      Res["data-label-name"] = dat["d"][0]["data"]
     }else if(dat["s"] == "label")
     {
      Res["data-is-label-to"] = "true"
      Res["data-label-name"] = dat["d"][0]["data"]
     }
     return Res
 ...
 function prepare_body(dat, task_name, is_modern = false)
 {
   try
   {
     if(task_name == "Initialize")
    ...
    if(dat === null)
     ...
     return BrowserAutomationStudio_GenerateActionText("", dat, 50, false, is_modern);
...
function BrowserAutomationStudio_GenerateActionText(action, data, max_len, no_html, is_modern)
{
   var State = data["s"]
   ...
   for(var i = 0;i<Split.length;i++)
   {
      if(i%2 == 0)
      {
         ...
         }else
         {
            ...
            if(LastArrow)
               Res += _.escape(tr(Split[i])).replace("-&gt;", is_modern ? `<img src="icons/output.svg" draggable="false">` : "<i class='fa fa-arrow-circle-right' aria-hidden='true'></i>")
