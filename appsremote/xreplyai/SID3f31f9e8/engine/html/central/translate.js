_L = 
{
	...
	"It allows to add or remove browser features for part of users. If it is missing, may be detected.":{"ru": "Эта технология позволяет включать или отключать функциональность браузера для части пользователей. Отсутствие поддержки variations может быть обнаружено."},

	"BAS helper": {"ru": "Помощник BAS"},
	"Autostart settings": {"ru": "Настройки автозапуска"},
	"Start manually": {"ru": "Запускать вручную"},
	"Autostart": {"ru": "Автозапуск"},
	"Autostart locally": {"ru": "Автозапуск локально"},
	"In order to start helper normally, click here:": {"ru": "Чтобы запустить помощника в обычном режиме, нажмите:"},
	"start helper": {"ru": "запустить помощника"},
	"to run local version, click:": {"ru": "чтобы запустить локальную версию, нажмите:"},
	"start helper locally": {"ru": "запустить локально"},
	"BAS helper allows you to write BAS scripts by entering text prompts. At the moment the project is under development, bugs are possible, not all modules are supported. Starting helper locally means that BAS helper will connect to local server, so this server must be launched manually in advance. Local mode is used mostly for development.": {"ru": "Помощник позволяет создавать скрипты для BAS, используя текстовые запросы. В настоящее время проект находится в стадии разработки, возможны ошибки, не все модули поддерживаются. Запуск помощника локально означает, что он будет подключаться к локальному серверу, поэтому этот сервер должен быть заранее запущен вручную. Локальный режим в основном используется для разработки."},
}
