...
{
	_ensure_smtp_client();
	_SMTP_SETTINGS.options["CURLOPT_USE_SSL"] = (encrypt == "none" ? 0 : 3);
	host = "smtp" + (["ssl", "tls"].indexOf(encrypt) > -1 ? "s" : "") + "://" + host
...
function smtp_client_send_mail(mailfrom, mailto, mailcc, mailsubject, mailbody, is_html, attachments)
{
	...
	if(_SMTP_SETTINGS.options["CURLOPT_USERNAME"] && (!mailfrom || mailfrom.indexOf("@") < 0)){
		_SMTP_NEW_MAIL.options["CURLOPT_MAIL_FROM"] = _SMTP_SETTINGS.options["CURLOPT_USERNAME"];
	}else{
		if(!mailfrom)
			fail("Mail From not specified")
		
		var fromstart = mailfrom.indexOf("<") + 1;
		
		if(fromstart == 0){
			_SMTP_NEW_MAIL.options["CURLOPT_MAIL_FROM"] = mailfrom;
		}else{
			var fromend = mailfrom.indexOf(">", fromstart);
			_SMTP_NEW_MAIL.options["CURLOPT_MAIL_FROM"] = mailfrom.slice(fromstart, fromend);
		}
	}
	
	if(_SMTP_NEW_MAIL.options["CURLOPT_MAIL_FROM"].indexOf("@") < 0)
		fail("Mail From must contain the domain")
	...
	var fromheader = mailfrom;
	if(!mailfrom){
		fromheader = "<" + _SMTP_SETTINGS.options["CURLOPT_USERNAME"] + ">"
	}else if(mailfrom.indexOf("<") < 0){
		if(_SMTP_SETTINGS.options["CURLOPT_USERNAME"] && mailfrom != _SMTP_SETTINGS.options["CURLOPT_USERNAME"]){
			fromheader = mailfrom + " <" + _SMTP_SETTINGS.options["CURLOPT_USERNAME"] + ">"
		}else{
			fromheader = "<" + mailfrom + ">"
		}
	}
