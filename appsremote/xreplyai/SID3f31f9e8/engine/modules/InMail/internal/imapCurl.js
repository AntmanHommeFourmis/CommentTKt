_InMail.imap = _InMail.assignApi(function(config){
	...
	this.makeRequest = function(){
		...
		var timeout = _avoid_nilb(_function_argument("timeout"), 60000);
		var maxTime = _avoid_nilb(_function_argument("maxTime"), Date.now() + timeout);
		var utf8IsEnabled = false;
		
		_if(isUTF8, function(){
			_call_function(api.capability, {timeout: timeout, maxTime: maxTime})!
			
			_if(api.serverSupports('ENABLE') && api.serverSupports('UTF8=ACCEPT'), function(){
				_if(!api.utf8 || (api.resetTime && api.resetTime < Date.now()), function(){
					_call_function(api.request, {query: 'ENABLE UTF8=ACCEPT', timeout: timeout, maxTime: maxTime})!
					api.utf8 = utf8IsEnabled = true;
				})!	
				query = query.replace(' CHARSET UTF-8', '');
			})!		
		})!
		
		_call_function(api.request, {query: query, path: path, checkErrors: false, timeout: timeout, maxTime: maxTime})!
		var resp = _result_function();
		...
		_if_else(resp.code == "OK", function(){
			_function_return(resp.result);
		}, function(){
			var error = api.parseError(resp);
			
			_if_else(isUTF8 && !utf8IsEnabled && error == "Could not parse command", function(){
				api.utf8 = false;
				_call_function(api.makeRequest, {query: query, isUTF8: isUTF8, box: box, timeout: timeout, maxTime: maxTime})!
