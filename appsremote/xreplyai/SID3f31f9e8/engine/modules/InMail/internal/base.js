_InMail.baseApi = function(isCurl, protocol, config){
	...
	if(isCurl){
		...
		this.request = function(){
			...
			var resetTimeout = api.resetTimeout || 5 * 60 * 1000;
			
			_call_function(api.wrapper, {write_to_string: true, options: options, trace: true, is_fetch: isFetch, save_session: true, timeout: resetTimeout})!
			var resp = _result_function();
			
			api.resetTime = Date.now() + resetTimeout;
			...
			if(resp.code == "OK"){
				...
				api.errorHandler(resp.code, api.parseError(resp));
			};
		};
